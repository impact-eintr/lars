...
// 因为没有连接，所以都是以包为单位来处理的，一个包一个包处理。可能相邻的两个包来自不同的客户端。
void udp_server::do_read(){
  while(true) {
    ...
    if (head.msglen > MESSAGE_LENGTH_LIMIT || head.msglen < 0 ||
      ...
      // 报文格式有问题
      fprintf(stderr,
              "do_read, data error, msgid = %d, msglen = %d, pkg_len = %d\n",
...
int udp_server::send_message(const char*data, int msglen, int msgid) {
  ...
  int ret = sendto(_sockfd, _write_buf,
                 msglen + MESSAGE_HEAD_LEN, 0, (struct sockaddr*)&_client_addr, _client_addrlen);
  if (ret == -1) {
    perror("sendto()...");
    return -1;
  }
