...
// 定义一个存放配置信息的map
// key 是string存放一个标题的section
// value 是以 map 存放该标题下面所有的key-value键值对
...
class config_file {
  public:
    ...
    // 获取整形类型配置信息
    unsigned GetNumber(const std::string &section, const std::string &key,
    ...
    static config_file &instance() {
      static config_file config = config_file();
      return config;
    }
